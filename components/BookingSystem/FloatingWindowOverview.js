...
const FloatingWindow = ({
  ...
  const selectedServices = state.services.filter(({ id }) =>
    selectedServicesId.includes(id)
  );
  const stylistType = steps.step3.serviceType;
  const selectedServicesPrice = selectedServices
    .map((service) =>
  ...
  const stylist = stylistType
    ? state.stylists.artDirector.find((stylist) => stylist.id === stylistId)
    : state.stylists.stylist.find((stylist) => stylist.id === stylistId);

  const coupon = async (code) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/getPromocodeFromCode/${code}`
      );
  ...
  const applyCouponCode = async (e) => {
    ...
    if (codeFromServer.data !== undefined) {
      ...
      if (!toast.isActive(formToast.current.coupon)) {
        formToast.current.coupon = toast.error(
          "This code is not redeemable now!"
        );
      }
    }
  };
  ...
  return (
    <div className="floating-window">
      ...
      <div className="floating-window-body">
        <p className="mb-2">
          <strong>{state.text.bookingHairSize}: </strong>
          {state.locale == "en"
            ? optionHairSize[steps.step1.value].name_en
            : optionHairSize[steps.step1.value].name}
        </p>
        {steps.step2.value !== -1 && (
          <p className="mb-2">
            <strong>{state.text.bookingHairType}: </strong>
            {state.locale == "en"
              ? optionHairType[steps.step2.value].name_en
              : optionHairType[steps.step2.value].name}
          </p>
        )}
        <p className="mb-2">
        ...
        <p className="mb-2">
          ...
          {format(steps.step5.value, "eee PPpp", {
            locale: state.locale == "en" ? en : nl,
          })}
        </p>
        <p className="mb-1">
          ...
          <tbody>
            {selectedServices.map((service, index) => (
              <tr key={index}>
                ...
                <td>{state.locale == "en" ? service.name_en : service.name}</td>
                <td>
                ...
                <td>
                  {makePrice(
                    stylistType
                      ? service.art_director_price
                      : service.stylist_price
                  )}
                </td>
              </tr>
            ))}
            ...
            <div className="mb-3">
              ...
              <div className="input-group">
                <input
                  ...
                  id="coupon"
                  placeholder={state.text.bookingCouponCode}
                  value={couponCode}
                  onChange={(e) => setCouponCode(e.target.value)}
                />
                <button className="btn btn-dark" type="submit">
                  {state.text.bookingApply}
                </button>
              </div>
            </div>
          </form>
      ...
      <div className="floating-window-footer">
        <a className={`btn-next btn btn-dark`} onClick={previousStep}>
          <BiLeftArrowAlt className="me-1" />
          {state.text.bookingBack}
        </a>
        <a className={`btn-next btn btn-dark`} onClick={payLater} href="#">
          <BiDollarCircle className="me-1" />
          {state.text.bookingPayLater}
        </a>
        <a className="btn-next btn btn-dark" onClick={nextStep}>
          {state.text.bookingPayNow}
